...
class ChangelogPageTests(unittest.TestCase):
    ...
    def test_changelog_api_returns_structured_versions(self):
        ...
        self.assertEqual(latest['version'], 'v0.0.8')
        self.assertIn('updated_at', latest)
        self.assertIn('changes', latest)
        self.assertTrue(any('过程可视化' in item for item in latest['changes']))
        self.assertTrue(any(version['version'] == 'v0.0.7' for version in data['versions']))
