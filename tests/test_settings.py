...
class SettingsPageTests(unittest.TestCase):
    ...
    def test_settings_api_returns_safe_structured_configuration(self):
        ...
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data['version'], 'v0.0.8')
        ...
        self.assertEqual(data['activity_endpoint'], '/api/activity')
        self.assertNotEqual(data['api_key'], 'sk-tes...alue')
        self.assertIn('...', data['api_key'])
        self.assertIn('changelog', data)
        self.assertEqual(data['changelog'][0]['version'], 'v0.0.8')
        self.assertTrue(any(version['version'] == 'v0.0.7' for version in data['changelog']))
        self.assertTrue(any(version['version'] == 'v0.0.6' for version in data['changelog']))
